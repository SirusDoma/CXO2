...
namespace Gx
{
    namespace priv
    {
        ...
        struct fn_def {
            friend inline auto& loophole(tag<T, N>) { return activator<U>::activate(); }
            constexpr friend inline int cloophole(tag<T, N>) { return 0; }
        };
    ...
    // Indirection via traits to allow explicit specializations for problematic types on certain compilers.
    template <typename T>
    struct ConstructorDescriptorTraits
    {
        using type = typename priv::loophole_tuple<T, std::make_integer_sequence<int, GetConstructorParameterCount<T>(0)>>::type;
    };

    template <typename T>
    using ConstructorDescriptor = typename ConstructorDescriptorTraits<T>::type;
    ...
    namespace priv
    {
        ...
        template <typename T>
        struct Constructible<T, true>
        {
            static constexpr bool value = !std::is_abstract_v<T>;
        };

        template <typename T>
        struct Constructible<T, false>
        {
            static constexpr bool value = !std::is_abstract_v<T> && (GetConstructorParameterCount<T>(0) < priv::MaxParameterCount);
        };
    }
    ...
    struct Constructible
    {
        static constexpr bool value = priv::Constructible<T, std::is_default_constructible_v<T>>::value;
    };
