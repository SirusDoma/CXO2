...
void StateRoom::Initialize()
{
    ...
    const auto sfxAccept   = Instantiate<sf::Sound>("STATE_ROOM/IDC_SOUND_02");
    const auto sfxNavigate = Instantiate<sf::Sound>("STATE_ROOM/IDC_SOUND_07");
    const auto sfxToggle   = Instantiate<sf::Sound>("STATE_ROOM/IDC_SOUND_14");

    const auto player = session.GetCurrentPlayer();
    const auto nicknameLabel = Instantiate<Gx::Label>("STATE_ROOM/IDC_TEXT_NICKNAME");
    nicknameLabel->SetString("Lv." + std::to_string(player.Level) + ": " + player.Name);

    const auto avatar = Instantiate<Avatar>("STATE_ROOM/IDC_AVATAR");
    ...
    const auto channelNumber = Instantiate<Gx::Number>("STATE_ROOM/IDC_NUMBER_CHANNEL_ID");
    channelNumber->SetValue(session.GetChannelID());

    const auto chatPanel = Instantiate<ChatPanel>("STATE_ROOM/IDC_CHAT_PANEL");
    ...
    const auto roomContainer = Instantiate<RoomContainer>("STATE_ROOM/IDC_ROOM_CONTAINER");
    ...
    const auto createRoomButton = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_CREATE_ROOM");
    if (const auto dialog = Instantiate<Gx::Dialog>("STATE_ROOM/IDC_DIALOG_CREATE_ROOM"); dialog)
    {
    ...
    const auto showAllButton     = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_SHOW_ALL");
    const auto waitingRoomButton = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_SHOW_WAITING");
    ...
    const auto roomLeftButton  = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_ROOM_LEFT");
    const auto roomRightButton = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_ROOM_RIGHT");
    ...
    if (const auto dialog = Instantiate<Gx::Dialog>("STATE_ROOM/IDC_DIALOG_OPTION"); dialog)
    {
    ...
    const auto backButton = Instantiate<Gx::Button>("STATE_ROOM/IDC_BUTTON_BACK");
