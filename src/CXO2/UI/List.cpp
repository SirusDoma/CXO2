...
namespace Cx
{
    ...
    Gx::RenderStates List::Render(Gx::RenderSurface& surface, Gx::RenderStates states) const
    {
        if (!IsVisible())
            return states;
    ...
    void List::OnChildAdded(Gx::Node& node)
    {
        if (m_layouts.empty())
        {
    ...
    void List::OnChildRemove(Gx::Node& node)
    {
        if (!m_layouts.empty())
        {
