...
namespace Cx
{
    ...
    void ChatPanel::Initialize()
    {
        ...
        {
            if (const auto scrollChat = controls->FindChild<ScrollBar>(Resource::ChatPanel::IDC_SCROLL_BAR_CHAT))
                chatWindow->SetScrollBar(*scrollChat);

            if (const auto btnChatScrollUp = controls->FindChild<Cx::Button>(Resource::ChatPanel::IDC_BUTTON_SCROLL_UP))
                btnChatScrollUp->SetClickCallback([this] (auto& sender, auto& ev) { OnChatScrollUpButtonClicked(sender, ev); });

            if (const auto btnChatScrollDown = controls->FindChild<Cx::Button>(Resource::ChatPanel::IDC_BUTTON_SCROLL_DOWN))
                btnChatScrollDown->SetClickCallback([this] (auto& sender, auto& ev) { OnChatScrollDownButtonClicked(sender, ev); });
        }

        if (const auto chatButtonList = FindChild<List>(Resource::ChatPanel::IDC_LIST_CHAT_BUTTON))
        {
            const auto btnChatAll     = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_ALL);
            const auto btnChatFriend  = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_FRIEND);
            const auto btnChatGuild   = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_GUILD);
            const auto btnChatWhisper = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_WHISPER);
        ...
        const auto chatInput = FindChild<Cx::InputField>(Resource::ChatPanel::IDC_EDIT_CHAT);
    ...
    void ChatPanel::OnChatScrollUpButtonClicked(Control& sender, Control::Event& ev)
    {
        const auto controls   = FindChild<UiContainer>(Resource::ChatPanel::IDC_CONTAINER_CHAT_SCROLL_CONTROLS);
        const auto scrollChat = controls->FindChild<ScrollBar>(Resource::ChatPanel::IDC_SCROLL_BAR_CHAT);

        scrollChat->Decrease();
    }

    void ChatPanel::OnChatScrollDownButtonClicked(Control& sender, Control::Event& ev)
    {
        const auto controls   = FindChild<UiContainer>(Resource::ChatPanel::IDC_CONTAINER_CHAT_SCROLL_CONTROLS);
        const auto scrollChat = controls->FindChild<ScrollBar>(Resource::ChatPanel::IDC_SCROLL_BAR_CHAT);
    ...
    void ChatPanel::OnChatAllFocusChanged(Control& sender, Control::Event& ev)
    {
        ShowChatButtonToolTip(sender, Constants::Messages::Chat::Tooltips::ALL);
    }

    void ChatPanel::OnChatAllCheckChanged(Cx::RadioButton& radio, Control::Event& ev)
    {
        if (radio.IsChecked())
            m_recipient = {};
    }

    void ChatPanel::OnChatFriendFocusChanged(Control& sender, Control::Event& ev)
    {
        ShowChatButtonToolTip(sender, Constants::Messages::Chat::Tooltips::FRIEND);
    }

    void ChatPanel::OnChatGuildFocusChanged(Control& sender, Control::Event& ev)
    {
        ShowChatButtonToolTip(sender, Constants::Messages::Chat::Tooltips::GUILD);
    }

    void ChatPanel::OnChatFallbackCheckChanged(Cx::RadioButton& radio)
    {
        const auto chatButtonList = FindChild<List>(Resource::ChatPanel::IDC_LIST_CHAT_BUTTON);
        const auto btnChatAll     = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_ALL);
    ...
    void ChatPanel::OnChatWhisperFocusChanged(Control& sender, Control::Event& ev)
    {
        ShowChatButtonToolTip(sender, Constants::Messages::Chat::Tooltips::WHISPER);
    }

    void ChatPanel::OnChatWhisperCheckChanged(Cx::RadioButton& radio, Control::Event& ev)
    {
        ...
        if (radio.IsChecked())
        {
            if (const auto dialog = parent->Instantiate<Cx::Dialog>(Resource::ChatPanel::IDC_DIALOG_WHISPER); dialog)
            {
                if (const auto nicknameInput = dialog->FindChild<Cx::InputField>(Resource::ChatPanel::IDC_EDIT_NICKNAME))
                {
    ...
    void ChatPanel::OnWhisperDialogAccepted(Cx::InputField& nicknameInput, Cx::RadioButton& radio)
    {
    ...
    void ChatPanel::OnChatInputTextEntered(Cx::InputField& sender, Cx::InputField::TextEnteredEvent& ev)
    {
        ...
        if (Gx::StringHelper::StartsWith(text.toAnsiString(), Constants::Messages::Chat::Commands::WHISPER))
        {
            ...
            if (const auto chatButtonList = FindChild<List>(Resource::ChatPanel::IDC_LIST_CHAT_BUTTON))
            {
                if (const auto btnChatWhisper = chatButtonList->FindChild<Cx::RadioButton>(Resource::ChatPanel::IDC_RADIO_CHAT_WHISPER))
                {
                ...
                {
                    if (const auto dialog = parent->Instantiate<Cx::Dialog>(Resource::ChatPanel::IDC_DIALOG_WHISPER))
                    {
                        if (const auto nicknameInput = dialog->FindChild<Cx::InputField>(Resource::ChatPanel::IDC_EDIT_NICKNAME))
                            nicknameInput->SetString(m_recipient);
                    }
            ...
            m_service.SendWhisper(WhisperMessageRequest{recipient, message}, [=] (const MessageEnvelope<WhisperMessageResponse>& ev)
            {
        ...
        else if (Gx::StringHelper::StartsWith(text.toAnsiString(), Constants::Messages::Chat::Commands::ANNOUNCE) && m_session.GetRole() == Role::Administrator)
        {
            ...
            m_service.SendAnnouncement(AnnouncementRequest{message});
        }
    ...
    void ChatPanel::ShowChatButtonToolTip(const Control& sender, const sf::String& message)
    {
        if (const auto tooltip = FindChild<Cx::ToolTip>(Resource::ChatPanel::IDC_TOOLTIP_INFO))
        {
    ...
    {
        if (const auto chatBox = FindChild<Cx::InputField>(Resource::ChatPanel::IDC_EDIT_CHAT); chatBox)
