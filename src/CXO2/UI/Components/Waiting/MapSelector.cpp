...
namespace Cx
{
    ...
    void MapSelector::Initialize()
    {
        ...
        const auto map       = FindChild<Image>(Resource::Map::IDC_IMAGE_MAP);
        const auto randomMap = FindChild<Image>(Resource::Map::IDC_IMAGE_RANDOM_MAP);
        map->SetFrame(0);
        ...
        const auto mapName = FindChild<Label>(Resource::Map::IDC_TEXT_MAP_NAME);
        ...
        const auto mapLeftButton = FindChild<Cx::Button>(Resource::Map::IDC_BUTTON_MAP_LEFT);
        mapLeftButton->SetClickCallback([this] (auto& sender, auto& ev) { OnMapLeftButtonClicked(sender, ev); });

        const auto mapRightButton = FindChild<Cx::Button>(Resource::Map::IDC_BUTTON_MAP_RIGHT);
        mapRightButton->SetClickCallback([this] (auto& sender, auto& ev) { OnMapRightButtonClicked(sender, ev); });

        if (const auto mapLeftCover = FindChild<Image>(Resource::Map::IDC_IMAGE_COVER_LEFT))
            mapLeftCover->SetVisible(false);

        if (const auto mapRightCover = FindChild<Image>(Resource::Map::IDC_IMAGE_COVER_RIGHT))
            mapRightCover->SetVisible(false);

        const auto mapEffectTopButton = FindChild<Cx::RadioButton>(Resource::Map::IDC_RADIO_MAP_SELECT_TOP);
        mapEffectTopButton->SetCheckStateChangeCallback([this] (auto& sender, auto& ev) { OnMapSelectTopCheckChanged(sender, ev); });

        const auto mapEffectBottomButton = FindChild<Cx::RadioButton>(Resource::Map::IDC_RADIO_MAP_SELECT_BOTTOM);
    ...
    {
        if (const auto leftButton = FindChild<Cx::Button>(Resource::Map::IDC_BUTTON_MAP_LEFT))
        {
            leftButton->SetEnabled(enabled);
            leftButton->SetVisible(enabled);
        }

        if (const auto rightButton = FindChild<Cx::Button>(Resource::Map::IDC_BUTTON_MAP_RIGHT))
        {
            rightButton->SetEnabled(enabled);
            rightButton->SetVisible(enabled);
        }

        if (const auto leftCover = FindChild<Image>(Resource::Map::IDC_IMAGE_COVER_LEFT))

            leftCover->SetVisible(!enabled);

        if (const auto rightCover = FindChild<Image>(Resource::Map::IDC_IMAGE_COVER_RIGHT))
    ...
    void MapSelector::OnMapLeftButtonClicked(Control& sender, Control::Event& ev)
    {
        SetMapID(static_cast<int>(m_mapID) - 1);
    }

    void MapSelector::OnMapRightButtonClicked(Control& sender, Control::Event& ev)
    {
        SetMapID(static_cast<int>(m_mapID) + 1);
    }

    void MapSelector::OnMapSelectTopCheckChanged(Cx::RadioButton& sender, Control::Event& ev)
    {
        if (sender.IsChecked())
            SetEffectID(1);
    }

    void MapSelector::OnMapSelectBottomCheckChanged(Cx::RadioButton& sender, Control::Event& ev)
    {
        if (sender.IsChecked())
            SetEffectID(2);
    }
