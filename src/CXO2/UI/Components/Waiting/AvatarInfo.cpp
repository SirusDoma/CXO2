...
namespace Cx
{
    ...
    Gauge* AvatarInfo::GetLifeBar() const
    {
        return FindChild<Gauge>(Resource::Avatar::Info::IDC_GAUGE_AVATAR_INFO_LIFE);
    }
    ...
    void AvatarInfo::Invalidate()
    {
        ...
        if (const auto readyIndicator = FindChild<Image>(Resource::Avatar::Info::IDC_IMAGE_AVATAR_READY_INDICATOR); readyIndicator)
            readyIndicator->SetVisible(m_slot && m_slot->Ready);

        const auto name  = FindChild<Label>(Resource::Avatar::Info::IDC_TEXT_AVATAR_INFO_NAME);
        const auto level = FindChild<Label>(Resource::Avatar::Info::IDC_TEXT_AVATAR_INFO_LEVEL);
