...
namespace Cx
{
    ...
    void PlayMenu::Initialize()
    {
        ...
        m_title    = FindChild<Label>(Resource::Game::Menu::IDC_TEXT_MUSIC_TITLE);
        ...
        m_wave     = FindChild<Gauge>(Resource::Game::Menu::IDC_GAUGE_WAVE);
        m_minutes  = FindChild<BitmapNumber>(Resource::Game::Menu::IDC_NUMBER_PLAY_TIME_MINUTE);
        m_seconds  = FindChild<BitmapNumber>(Resource::Game::Menu::IDC_NUMBER_PLAY_TIME_SECOND);
        m_bgmVol   = FindChild<Gauge>(Resource::Game::Menu::IDC_GAUGE_VOLUME_MUSIC);
        m_sfxVol   = FindChild<Gauge>(Resource::Game::Menu::IDC_GAUGE_VOLUME_EFFECT);

        if (const auto pointList = FindChild<List>(Resource::Game::Menu::IDC_LIST_NOTE_POINT_NUMBER); pointList)
        {
            const auto children = pointList->GetChildren();
            for (std::size_t i = 0; i < children.size(); i++)
            {
                auto acc = static_cast<Accuracy>(i);
                m_counters[acc] = dynamic_cast<BitmapNumber*>(children[i]);
            }
        }
    }
