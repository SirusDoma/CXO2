...
namespace Cx
{
    ...
    void ChannelBoard::Initialize()
    {
        ...
        const auto container = FindChild<Cx::UiContainer>(Resource::Planet::ChannelBoard::IDC_CONTAINER_CHANNEL_CONTROLS);

        const auto channelTabButton = FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_CHANNEL_TAB);
        ...
        const auto noticeTabButton = FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_NOTICE_TAB);
        noticeTabButton->SetClickCallback([this] (auto& sender, auto& ev)
        {
            OnNoticeTabButtonClicked(sender, ev);
        });

        const auto channelList       = container->FindChild<List>(Resource::Planet::ChannelBoard::IDC_LIST_CHANNEL);
        const auto currentPageNumber = FindChild<BitmapNumber>(Resource::Planet::ChannelBoard::IDC_NUMBER_CURRENT_CHANNEL_PAGE);
        const auto maxPageNumber     = FindChild<BitmapNumber>(Resource::Planet::ChannelBoard::IDC_NUMBER_MAX_CHANNEL_PAGE);
        currentPageNumber->SetDigitCount(2);
        maxPageNumber->SetDigitCount(2);

        const auto enterButton = container->FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_CHANNEL_ENTER);
        ...
        const auto leftButton = FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_CHANNEL_LEFT);
        leftButton->SetClickCallback([this] (auto& sender, auto& ev)
        {
            OnChannelLeftButtonClicked(sender, ev);
        });

        const auto rightButton = FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_CHANNEL_RIGHT);
    ...
    void ChannelBoard::ShowNotice(unsigned int page)
    {
        ...
        auto currentPageNumber = FindChild<BitmapNumber>(Resource::Planet::ChannelBoard::IDC_NUMBER_CURRENT_CHANNEL_PAGE);
    ...
    void ChannelBoard::OnChannelTabButtonClicked(Control& sender, Control::Event& ev)
    {
    ...
    void ChannelBoard::OnNoticeTabButtonClicked(Control& sender, Control::Event& ev)
    {
    ...
    void ChannelBoard::OnChannelEnterButtonClicked(Control& sender, Control::Event& ev)
    {
    ...
    void ChannelBoard::OnChannelButtonClicked(Control& sender, Control::Event& ev)
    {
        const auto channelButton = dynamic_cast<ChannelButton*>(&sender);
        m_selectedChannel = channelButton->GetChannelNumber() - 1;
    }

    void ChannelBoard::OnChannelButtonDoubleClicked(Control& sender, Control::Event& ev)
    {
        const auto container   = FindChild<Cx::UiContainer>(Resource::Planet::ChannelBoard::IDC_CONTAINER_CHANNEL_CONTROLS);
        const auto enterButton = container->FindChild<Cx::Button>(Resource::Planet::ChannelBoard::IDC_BUTTON_CHANNEL_ENTER);
    ...
    void ChannelBoard::OnChannelLeftButtonClicked(Control& sender, Control::Event& ev)
    {
    ...
    void ChannelBoard::OnChannelRightButtonClicked(Control& sender, Control::Event& ev)
    {
        m_mixer.Play(*m_sfxNavigate, Sound::Channel::SFX);
        if (m_tab == Tab::ChannelList)
        {
    ...
    void ChannelBoard::OnChannelClicked(Control& sender, Control::Event& ev)
    {
