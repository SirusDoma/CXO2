...
namespace Cx
{
    BitmapNumber::BitmapNumber() :
        ...
        m_state(Gx::Animation::AnimationState::Initial),
        m_value(),
        m_digitCount(),
        m_width(),
        m_height(),
    ...
    Gx::BlendMode BitmapNumber::GetBlendMode() const
    {
        return m_blendMode;
    }
    ...
    Gx::Animation::AnimationState BitmapNumber::GetAnimationState() const
    {
        return m_state;
    }
    ...
    {
        m_state = Gx::Animation::AnimationState::Stopped;
        if (m_callback)
        {
    ...
    {
        if (m_state == Gx::Animation::AnimationState::Initial || m_state == Gx::Animation::AnimationState::Playing)
        {
            for (auto& [digit, elapsed] : m_durations)
            {
                ...
                {
                    if (m_state != Gx::Animation::AnimationState::Playing)
                    {
                        m_state = Gx::Animation::AnimationState::Playing;
                        if (m_callback)
                        {
                            auto uiEvent = Event{false, GetControlState()};
                            m_callback(*this, uiEvent);
                    ...
                    {
                        m_state = Gx::Animation::AnimationState::Completed;
                        if (m_callback)
                        {
                            auto uiEvent = Event{false, GetControlState()};
                            m_callback(*this, uiEvent);
    ...
    Gx::RenderStates BitmapNumber::Render(Gx::RenderSurface& surface, Gx::RenderStates states) const
    {
        if (!IsVisible())
            return states;
        ...
        switch (m_blendMode)
        {
            case Gx::BlendMode::Alpha:          states.blendMode = sf::BlendAlpha;    break;
            case Gx::BlendMode::Additive:       states.blendMode = sf::BlendAdd;      break;
            case Gx::BlendMode::Multiplicative: states.blendMode = sf::BlendMultiply; break;
            case Gx::BlendMode::Min:            states.blendMode = sf::BlendMin;      break;
            case Gx::BlendMode::Max:            states.blendMode = sf::BlendMax;      break;
            case Gx::BlendMode::None:           states.blendMode = sf::BlendNone;     break;
            case Gx::BlendMode::Auto:                                                 break;
        }
