...
namespace Cx
{
    Dialog::Dialog(Dialog&& other) noexcept :
        Gx::Node(std::move(other)),
        UiContainer(std::move(other)),
        Gx::Sprite(std::move(other)),
    ...
    Dialog& Dialog::operator=(Dialog&& other) noexcept
    {
        if (this != &other)
        {
            Gx::Node::operator=(std::move(other));
            UiContainer::operator=(std::move(other));
            Gx::Sprite::operator=(std::move(other));
    ...
    Gx::Presentable::Parent* Dialog::GetPresentableParent() const
    {
        return m_parent;
    }
    ...
    void Dialog::OnPresented(Parent& parent, const Gx::PresentationContext& context)
    {
        if (m_shown)
            return;

        if (const auto ctx = dynamic_cast<const Gx::GraphicalPresentationContext*>(&context))
        {
            ...
            if (const auto dctx = dynamic_cast<const DialogPresentationContext*>(ctx))
            {
                if (dctx->UseBackdrop)
                {
                    m_backdrop = Gx::Rectangle(ctx->Bounds.size);
                    m_backdrop.SetColor(sf::Color(0, 0, 0, 255 / 2));
                }
                else
                    m_backdrop = Gx::Rectangle(sf::Vector2f(0, 0));
    ...
    Gx::RenderStates Dialog::Render(Gx::RenderSurface& surface, Gx::RenderStates states) const
    {
        if (!IsVisible())
            return states;
        ...
        return Gx::Sprite::Render(surface, states);
    }
